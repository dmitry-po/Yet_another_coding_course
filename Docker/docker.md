 ...
 * -t, --tty - выделяет псевдо-терминал для работы  с STDIN STDOUT (комбинацию флагов -i -t можно заменить -it).
 ...
 `docker volume [COMMAND]` - работа с томами (подробнее в разделе ["Работа с данными"](#Работа-с-данными)). Основные команды:
  * `create [OPTIONS] [VOLUME]` - создание нового тома;
  * `inspect [OPTIONS] VOLUME [VOLUME...]` - вывод детальной информации о томах;
  * `ls [OPTIONS]` - вывод списка имеющихся томов (перечень опций включает флаги *--filter* и *--quiet*, аналогично работе с контейнерами и образами);
  * `rm [OPTIONS] VOLUME [VOLUME...]` - удаление томов. Удалить можно тома, который не связаны ни с одним контейнером.

## Работа с данными
Docker рекомендуется использовать для запуска stateless-приложений, которые не требуют постоянного хранения информации о своё текущем состоянии, так как по умолчанию все данные, которые создаются и обрабатываются в контейнере, остаются в контейнере, непосредственно в слое с исполняемым приложением. Жизненный цикл данных в таком случае тесно связан с жизненным циклом контейнера: данные могут пережить остановку контейнера, но не его удаление. Доступ к данным, формируемым в ходе выполнения приложения, также ограничен пределами самого контейнера.  
Тем не менее, в Docker реализован функционал, который позволяет организовать долговременное хранение, а также совместный доступ к информации для statefull-приложений: *bind mount*, *volumes*, *tmpfs* (для Linux) и *named pipes* (для Windows).  
![Ways of storing data with Docker](./types-of-mounts.png)

1. **bind mount** реализует механизм монтирования выбранного каталога (файла) с хоста в контейнер, при этом к каталогу (файлу) одновременно получают доступ как процессы хоста, так и процессы контейнера. Данный подход удобно использовать для подключения конфигурационных файлов, но лучше не применять в продакшене: из-за совместного доступа к ресурсам можно перезатереть изменения. Также следует учитывать, что путь к выбранному каталогу (файлу) на хосте должен быть указан в явном виде и не должен меняться в процессе использования контейнера.  
Для использования *bind mount* необходимо в команду `docker run` передать параметр *--mount* с указанием:
   * типа монтирования (*type=bind*);
   * абсолютного пути к каталогу (файлу) на хосте (по ключу *source* или *src*);
   * пути к каталогу (файлу) в контейнере (по ключу *destination*, *dst* или *target*);
   * прочих опции по необходимости (например, *readonly* для ограничить возможность записи в каталог из контейнера):  
`docker run --mount type=bind,source="C:/Docker/bind-mount-test",target=/app/bind-mount`  

   При монтировании с помощью *bind mount* следует также помнить, что:
   * Если в контейнере не существует каталог *target*, он будет автоматически создан;
   * Если в контейнере уже существует каталог *target* и он не пустой, его содержимое будет игнорироваться в пользу каталога хоста (*source*).

2. Использование томов (**volumes**) предполагает создание специального, выделенного каталога в файловой директории Docker за пределами контейнеров. Созданием и управление данных томов (*volumes*) занимается Docker. Разработчики Docker рекомендуют именно данный способ хранения данных, что не лишено смысла, так как тома обладают рядом полезных свойств:
   * Являются средством постоянного хранения данных;
   * Независимы от контейнеров - это позволит, во-первых, сохранить информацию (тома) даже после удаления всех связных контейнеров, а во-вторых, разделить места хранения контейнера и данных (использование выделенного тома не увеличит дисковое пространство, занимаемое контейнером);
   * Могут быть использованы несколькими контейнерами (соответственно, тома можно использовать для обмена информацией между контейнерами);
   * Не зависят от структуры каталогов и ОС хоста и могут быть вынесены в облачное хранилище.

   Создать том Docker можно несколькими способами:
   * Прописать инструкцию вида `VOLUME /data-dir` в Dockerfile. При сборке образа будет создан том, в который будет скопировано содержимое каталога *data-dir*. Однако, использование такой инструкции не освобождает от необходимости указать данную точку монтирования при запуске контейнера;
   * Вручную из командной строки при помощи команды вида `docker volume create --name <VOLUME-NAME>` (если имя не задано, оно будет сгенерировано автоматически).  

   Для присоединения тома к контейнеру необходимо в команду `docker run` передать параметр *--mount* с указанием:
   * типа монтирования (*type=volume*);
   * наименования тома (по ключу *source* или *src*). Если используется неименованный том, данную пару ключ-значение указывать не нужно;
   * пути к каталогу в контейнере (по ключу *destination*, *dst* или *target*);
   * прочих опций по необходимости:  
`docker run --mount type=volume,source=demo_volume,target=/app/volume-mount`  

3. При использовании **tmpfs** данные будут храниться в специально отведенной области оперативной памяти хоста, что позволяет достичь более высокой скорости чтения/записи. Подходит только для временного хранения, так как удаление происходит не только вместе с контейнером, но и в момент его остановки. Из-за своих особенностей, данный способ хранения может быть рекомендован к использованию в случае работы с чувствительной информацией, которая может/должна быть удалена при завершении работы приложения. Такое хранилище может одновременно работать только с 1 контейнером.  
Для использования *tmpfs* необходимо в команду `docker run` передать параметр *--mount* с указанием:
   * типа монтирования (*type=tmpfs*);
   * пути к каталогу в контейнере (по ключу *destination*, *dst* или *target*);
   * опций tmpfs-size (ограничение размера) и tmpfs-mode по необходимости:  
`docker run --mount type=volume,source=demo_volume,target=/app/volume-mount`  
